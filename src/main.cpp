...
bool splashonclear = false;                         // if true, splash screen will be displayed each time ClearScreen invoked
bool firstpass = true;                              // this flag is true oncec during startup
uint64_t splashdelay = 0;
#endif
bool startupdisplayoff = false;                     // if true during startup the screen stays OFF until SIGUSR2
bool ignoreredrawarea = false;                      // if true, screen goes OFF after "turn_off_after" and stays OFF until SIGUSR2
bool displayOffForced = false;                      // if true, screen stays OFF until SIGUSR2
bool displayIsActive = false;                       // true if the redraw area is bigger than DISPLAY_CONSIDERED_INACTIVE_PERCENTAGE
bool displayOff = false;                            // if true, display driver is in OFF state
bool backlightOff = false;                          // if true, backlight is OFF
bool splashTimeout = false;                         // if true, all changes to framebuffer are supressed
volatile bool programRunning = true;                // if set to false program termination invoked 
uint64_t turnoffafter = 0;
...
static void ProgramInterruptHandler(int signal)
{
    switch (signal)
    {
        ...
        case SIGUSR1:
            ...
            if(!backlightOff) {
                TurnBacklightOff();
                LOG("- backlight OFF via SIGUSR1");
                backlightOff = true;
            } else {
                TurnBacklightOn();
                LOG("- backlight ON via SIGUSR1");
                backlightOff = false;
            }
#else 
            LOG("- not implemented");
        ...
        case SIGUSR2:
            ...
            if(!displayOff) {
                TurnDisplayOff();
                LOG("- display forced to stay OFF");
                displayOffForced = true;
                displayOff = true;
                backlightOff = true;
            } else {
                TurnDisplayOn();
                LOG("- display switched back ON");
                displayOffForced = false;
                displayOff = false;
                backlightOff = false;
                // used if ignoreredrawarea = true
        ...
        case SIGPROF:
            ClearScreen();
            LOG("- clear screen via SIGPROF");
            break;
        case SIGKILL:
            LOG("SIGKILL fbcpX invoked, performing a forcible shutdown!");
            exit(1);
        case SIGTERM:
        case SIGQUIT:
        case SIGINT:
...
/**
 * @brief Could be merged with SIGQUIT
 * 
*/
inline static void unifiedTerminator() {
    // does this already unloaded
    if(splashonclear) {
        unloadSplash();
    }
    DeinitGPU();
...
int main()
{
    // self identify
    PRINT_VERSION();
    // @fbcpX: configuration begin
    fs::path cf("/etc/fbcpx/fbcpX.conf");
    if (!fs::exists(cf))
    {
        LOG("(!) no configuration found in /etc/fbcpx/fbcpX.conf");
        return 1;
    }
    config = new ConfigFile(cf.string());
    
    uint32_t curFrameEnd, prevFrameEnd;
    ...
    startupdisplayoff = config->read<bool>("startup_display_off", false);
    LOG("- startup_display_off = %d", startupdisplayoff);
    ignoreredrawarea = config->read<bool>("ignore_redraw_area", false);
    LOG("- ignore_redraw_area = %d", ignoreredrawarea);
    
#ifdef TURN_DISPLAY_OFF_AFTER_USECS_OF_INACTIVITY
    turnoffafter = config->read<uint64_t>("turn_off_after", TURN_DISPLAY_OFF_AFTER_USECS_OF_INACTIVITY);
    LOG("- turn_off_after = %llu sec", turnoffafter);
    LOG("- min redraw area = %.2f", (float)DISPLAY_CONSIDERED_INACTIVE_PERCENTAGE);
    // µsec
    turnoffafter *= 1000000ULL;
#endif
    // @fbcpX: configuration end
    // @fbcpX: load splash
#ifdef USE_SPLASH_SCREEN
    if(!loadSplash()) {
        //NOTE cannot goto from here. Let's use an unified terminatiion function
        unifiedTerminator();
        return 1;
    }
#endif
    // @fbcpX: load splash end

    signal(SIGINT, ProgramInterruptHandler);
    signal(SIGQUIT, ProgramInterruptHandler);
    signal(SIGTERM, ProgramInterruptHandler);
    signal(SIGKILL, ProgramInterruptHandler);   // die hard
    // signal(SIGALRM, ProgramInterruptHandler);
    signal(SIGPROF, ProgramInterruptHandler);   // clear screen
    signal(SIGUSR1, ProgramInterruptHandler);   // toggle backlight
    signal(SIGUSR2, ProgramInterruptHandler);   // toggle display on/off, if GPIO_TFT_BACKLIGHT enabled
    ...
    OpenMailbox();
    InitSPI();              //NOTE triggers ClearScreen()
#ifdef USE_SPLASH_SCREEN
    firstpass = false;
...
#   endif
    ...
    // Main loop
    //
    LOG("All initialized, now running main loop...");
    while (programRunning)
    {
loopStart:
        // trigger ClearScreen via sub function
        ClearScreenTick();
        ...
        else
        {
            ...
            {
#if defined(GPIO_TFT_BACKLIGHT) && defined(TURN_DISPLAY_OFF_AFTER_USECS_OF_INACTIVITY)
                if (!displayOff && tick() - waitStart > turnoffafter)
                {
                    LOG("- inactive, sleep...");
                    TurnDisplayOff();
                    displayOff = true;
                ...
                {
                    timespec timeout = {};
                    timeout.tv_sec = (turnoffafter * 1000) / 1000000000;
                    timeout.tv_nsec = (turnoffafter * 1000) % 1000000000;
                    if (programRunning)
...
/*
        if (once && starved)
        {
            LOG("Had %u bytes in queue, asked to sleep for %u usecs, got %u usecs sleep, afterwards %u bytes in queue. (got %.2f%% work done)%s",
...
*/
...
#if !defined(NO_INTERLACING) || (defined(GPIO_TFT_BACKLIGHT) && defined(TURN_DISPLAY_OFF_AFTER_USECS_OF_INACTIVITY))
        ...
        if(!ignoreredrawarea) {
            ...
            displayIsActive = percentageOfScreenChanged > DISPLAY_CONSIDERED_INACTIVE_PERCENTAGE;
            if (displayIsActive)
                displayContentsLastChanged = tick();

            bool keyboardIsActive = TimeSinceLastKeyboardPress() < turnoffafter;
            if (displayIsActive || keyboardIsActive)
            {
                if (displayOff && !displayOffForced)
                {
                    LOG("- active - woke up");
                    TurnDisplayOn();
                    displayOff = false;
            ...
            else if (!displayOff && tick() - displayContentsLastChanged > turnoffafter)
            {
                LOG("- timeout - sleep...");
                TurnDisplayOff();
                displayOff = true;
            ...
            // ignore_redraw_area
            if (!displayOff && (tick() - displayContentsLastChanged) > turnoffafter) {
                LOG("- timeout, ignore_redraw_area - sleep...");
                TurnDisplayOff();
                displayOff = true;
                displayOffForced = true;
            }
