...
void InitST7735R()
{
    ...
    LOG("Resetting display at reset GPIO pin %d", GPIO_TFT_RESET_PIN);
...
#define MADCTL_ROTATE_180_DEGREES (MADCTL_COLUMN_ADDRESS_ORDER_SWAP | MADCTL_ROW_ADDRESS_ORDER_SWAP)
    ...
    LOG("Setting TFT backlight on at pin %d", GPIO_TFT_BACKLIGHT);
    SET_GPIO_MODE(GPIO_TFT_BACKLIGHT, 0x01); // Set backlight pin to digital 0/1 output mode (0x01) in case it had been PWM controlled
    // TODO @fbcpX leaving this as is for now since the display on/off 
    // control is implemented toggling backlight. use CMD:28/29
    if(!startupdisplayoff) {
        SET_GPIO(GPIO_TFT_BACKLIGHT);            // And turn the backlight on.
    } else {
        LOG("- display backlight is OFF - startup_display_off");
        // force pulldown pin in case it was enabled any time before 
