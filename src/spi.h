...
// NOTE: warning: taking address of packed member of ‘SPITask’ may result in an unaligned pointer value [-Waddress-of-packed-member] (__attribute__ ((packed)))
typedef struct __attribute__ ((aligned(4))) SPITask
{
...
#ifdef SPI_3WIRE_PROTOCOL
    inline uint8_t *PayloadStart()
    {
        return data + (size - sizeExpandedTaskWithPadding);
    }
...
#define QUEUE_SPI_TRANSFER(command, ...)                   \
    do                                                     \
    {                                                      \
        char data_buffer[] = {__VA_ARGS__};                \
        SPITask *t = AllocTask(sizeof(data_buffer));       \
...
#define QUEUE_MOVE_CURSOR_TASK(cursor, pos) \
    do                                      \
    {                                       \
...
#define QUEUE_SET_WRITE_WINDOW_TASK(cursor, x, endX) \
    do                                               \
    {                                                \
...
/**
 * @brief Returns a pointer to a new SPI task block, called on main thread
 * 
 * @param bytes 
 * @return SPITask* 
*/
static inline SPITask *AllocTask(uint32_t bytes) {
#ifdef SPI_3WIRE_PROTOCOL
