...
#define VERSION "- fbcpX ver. 45e77fe (mod. 20230203)"
...
#define PRINT_VERSION() do { LOG(VERSION); pr_alert(VERSION "\n"); } while(0)
#else
#define LOG(...) do { printf(__VA_ARGS__); printf("\n"); } while(0)
#define FATAL_ERROR(msg) do { fprintf(stderr, "%s\n", msg); syslog(LOG_ERR, msg); exit(1); } while(0)
#define PRINT_VERSION() do { LOG(VERSION); syslog(LOG_INFO, VERSION); } while(0)
#endif
