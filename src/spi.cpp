...
void DumpSPICS(uint32_t reg)
{
    ...
    LOG("SPI0 DLEN: %u", spi->dlen);
    LOG("SPI0 CE0 register: %d", GET_GPIO(GPIO_SPI0_CE0) ? 1 : 0);
}
...
void SetRealtimeThreadPriority()
{
    ...
    if (failed)
        FATAL_ERROR("pthread_setschedparam() failed!");

    int policy = 0;
    failed = pthread_getschedparam(pthread_self(), &policy, &params);
    if (failed)
        FATAL_ERROR("pthread_getschedparam() failed!");

    if (policy != SCHED_FIFO)
        FATAL_ERROR("Failed to set realtime thread policy!");
    LOG("Set fbcp-ili9341 thread scheduling priority to maximum (%d)", sched_get_priority_max(SCHED_FIFO));
}
...
#define BYTE_TO_BINARY(byte)       \
  ...
  LOG("Interleaving result: 8-bit task of size %d bytes became %d bytes:", task->size - task->sizeExpandedTaskWithPadding, task->sizeExpandedTaskWithPadding - SPI_9BIT_TASK_PADDING_BYTES);
  LOG("8-bit c" BYTE_TO_BINARY_PATTERN, BYTE_TO_BINARY(task->cmd));
  for(int i = 0; i < task->size - task->sizeExpandedTaskWithPadding; ++i)
    LOG("d" BYTE_TO_BINARY_PATTERN, BYTE_TO_BINARY(task->data[i]));
  LOG("\n9-bit ");
  for(int i = 0; i < task->sizeExpandedTaskWithPadding - SPI_9BIT_TASK_PADDING_BYTES; ++i)
    LOG(BYTE_TO_BINARY_PATTERN, BYTE_TO_BINARY(dst[i]));
  LOG("\n");
#endif
}
...
#define TASK_SIZE_TO_USE_DMA 4
    ...
    if (payloadSize >= TASK_SIZE_TO_USE_DMA && (task->cmd == DISPLAY_WRITE_PIXELS || task->cmd == DISPLAY_SET_CURSOR_X || task->cmd == DISPLAY_SET_CURSOR_Y))
    {
        ...
        //    LOG("DMA cmd=0x%x, data=%d bytes", task->cmd, task->PayloadSize());
        SPIDMATransfer(task);
    ...
    else
    {
        if (!previousTaskWasSPI)
        {
            ...
            //    LOG("SPI cmd=0x%x, data=%d bytes", task->cmd, task->PayloadSize());
...
#define BCM2835_SPI0_BASE 0x204000
    ...
    if (!bcm2835)
        FATAL_ERROR("Failed to map BCM2835 address!");
    ...
    LOG("bcm_host_get_peripheral_address: %p", bcm_host_get_peripheral_address());
    LOG("bcm_host_get_peripheral_size: %u", bcm_host_get_peripheral_size());
    LOG("bcm_host_get_sdram_address: %p", bcm_host_get_sdram_address());
    ...
    LOG("BCM core speed: current: %uhz, max turbo: %uhz. SPI CDIV: %d, SPI max frequency: %.0fhz", currentBcmCoreSpeed, maxBcmCoreTurboSpeed, SPI_BUS_CLOCK_DIVISOR, (double)maxBcmCoreTurboSpeed / SPI_BUS_CLOCK_DIVISOR);
    ...
    if (driverfd < 0)
        FATAL_ERROR("Could not open SPI ring buffer - kernel driver module not running?");
    ...
    close(driverfd);
    if (spiTaskMemory == MAP_FAILED)
        FATAL_ERROR("Could not mmap SPI ring buffer!");
    LOG("Got shared memory block %p, ring buffer head %p, ring buffer tail %p, shared memory block phys address: %p", (const char *)spiTaskMemory, spiTaskMemory->queueHead, spiTaskMemory->queueTail, spiTaskMemory->sharedMemoryBaseInPhysMemory);

#ifdef USE_DMA_TRANSFERS
    LOG("DMA TX channel: %d, DMA RX channel: %d", spiTaskMemory->dmaTxChannel, spiTaskMemory->dmaRxChannel);
#endif
