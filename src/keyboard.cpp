...
void OpenKeyboard()
{
  ...
  if (key_fd < 0) 
  LOG("Warning: cannot open keyboard input file " KEYBOARD_INPUT_FILE "! Try double checking that it exists, or reconfigure it in keyboard.cpp, or remove line '#define BACKLIGHT_CONTROL_FROM_KEYBOARD' in config.h if you do not want keyboard activity to factor into backlight control.");
#endif
}
...
int ReadKeyboard()
{
  ...
  do
  {
    ...
    if (bytesRead >= sizeof(struct input_event))
    {
      ...
      {
//        LOG("time: %d %d type: %d, code: %d, value: %d", ev.time.tv_sec, ev.time.tv_usec, ev.type, ev.code, ev.value);
        ++numRead;
      }
    }
  } while(bytesRead > 0);
