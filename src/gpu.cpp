...
bool SnapshotFramebuffer(uint16_t *destination)
{
    ...
    if (failed)
    {
        ...
        LOG("vc_dispmanx_snapshot() failed with return code %d! If this appears related to a change in HDMI/display resolution, see https://github.com/juj/fbcp-ili9341/issues/28 and https://github.com/raspberrypi/userland/issues/461 (try setting fbcp-ili9341 up as an infinitely restarting system service to recover)", failed);
        // MarkProgramQuitting();
    ...
    if (failed)
    {
        LOG("vc_dispmanx_resource_read_data failed with return code %d!", failed);
        // MarkProgramQuitting();
        programRunning = false;
        return false;
    }
...
uint64_t EstimateFrameRateInterval()
{
    ...
    if (timeNow - mostRecentFrame > 60000000)
    {
        histogramSize = 1;
        return 500000;
    } // if it's been more than one minute since last seen update, assume interval of 500ms.
    if (timeNow - mostRecentFrame > 5000000)
        return lastFramePollTime + 100000; // if it's been more than 5 seconds since last seen update, assume interval of 100ms.
#endif
...
#else
    if (histogramSize < 2)
        return 100000; // Frame histogram needs to have at least a few entries to bootstrap, if there's very few, either refresh rate is low, or fbcp-ili9341 just started
...
uint64_t PredictNextFrameArrivalTime()
{
    ...
    } // if it's been more than one minute since last seen update, assume interval of 100ms.
    if (timeNow - mostRecentFrame > 5000000)
        return lastFramePollTime + 100000; // if it's been more than 5 seconds since last seen update, assume interval of 100ms.
#endif
...
void InitGPU()
{
    ...
    if (!display)
        FATAL_ERROR("vc_dispmanx_display_open failed! Make sure to have hdmi_force_hotplug=1 setting in /boot/config.txt");
    DISPMANX_MODEINFO_T display_info;
    int ret = vc_dispmanx_display_get_info(display, &display_info);
    if (ret)
        FATAL_ERROR("vc_dispmanx_display_get_info failed!");
    ...
    LOG("DISPLAY_FLIP_ORIENTATION_IN_SOFTWARE: Swapping width/height to update display in portrait mode to minimize tearing.");
#endif
    ...
    */
    ...
    LOG("Relevant source display area size with overscan cropped away: %dx%d.", relevantDisplayWidth, relevantDisplayHeight);
    ...
    syslog(LOG_INFO, "GPU display is %dx%d. SPI display is %dx%d with drawable area of %dx%d. Applying scaling factor horiz=%.2fx & vert=%.2fx, xOffset: %d, yOffset: %d, scaledWidth: %d, scaledHeight: %d",
            display_info.width, display_info.height, DISPLAY_WIDTH, 
                DISPLAY_HEIGHT, DISPLAY_DRAWABLE_WIDTH, DISPLAY_DRAWABLE_HEIGHT, 
                    scalingFactorWidth, scalingFactorHeight, displayXOffset, 
                        displayYOffset, scaledWidth, scaledHeight);
    LOG("Source GPU display is %dx%d", display_info.width, display_info.height);
    LOG("Output SPI display is %dx%d with a drawable area of %dx%d", DISPLAY_WIDTH, DISPLAY_HEIGHT, DISPLAY_DRAWABLE_WIDTH, DISPLAY_DRAWABLE_HEIGHT);
    LOG("Applying scaling factor horiz=%.2fx & vert=%.2fx, xOffset: %d, yOffset: %d, scaledWidth: %d, scaledHeight: %d",
        scalingFactorWidth, scalingFactorHeight, displayXOffset, displayYOffset, scaledWidth, scaledHeight);

    uint32_t image_prt;
    LOG("Creating dispmanX resource of size %dx%d (aspect ratio=%f).",
           scaledWidth + excessPixelsLeft + excessPixelsRight,
           scaledHeight + excessPixelsTop + excessPixelsBottom,
           (double)(scaledWidth + excessPixelsLeft + excessPixelsRight) / (scaledHeight + excessPixelsTop + excessPixelsBottom));
#ifdef DISPLAY_FLIP_ORIENTATION_IN_SOFTWARE
...
#endif
    if (!screen_resource)
        FATAL_ERROR("vc_dispmanx_resource_create failed!");
    LOG("GPU grab rectangle is offset x=%d,y=%d, size w=%dxh=%d, aspect ratio=%f", excessPixelsLeft, excessPixelsTop, scaledWidth, scaledHeight, (double)scaledWidth / scaledHeight);
