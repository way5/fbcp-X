...
//  LOG("Allocated %u bytes of GPU memory for %s (bus address=%p). Total GPU memory used: %llu bytes", mem.sizeBytes, reason, (void*)mem.busAddress, totalGpuMemoryUsed);
  return mem;
}
...
volatile DMAChannelRegisterFile *GetDMAChannel(int channelNumber)
{
  ...
  {
    LOG("Invalid DMA channel %d specified!", channelNumber);
...
void DumpDMAPeripheralMap()
{
  for(int i = 0; i < BCM2835_NUM_DMA_CHANNELS; ++i)
  {
    ...
    LOG("DMA channel %d has peripheral map %d (is lite channel: %d, currently active: %d, current control block: %p)", i, (channel->cb.ti & BCM2835_DMA_TI_PERMAP_MASK) >> BCM2835_DMA_TI_PERMAP_SHIFT, (channel->cb.debug & BCM2835_DMA_DEBUG_LITE) ? 1 : 0, (channel->cs & BCM2835_DMA_CS_ACTIVE) ? 1 : 0, channel->cbAddr);
  }
}
...
void CheckDMAChannelNotStolen(int channelNumber, int expectedPeripheralMap)
{
  ...
  if (peripheralMap != expectedPeripheralMap && peripheralMap != 0)
  {
    ...
    LOG("DMA channel collision! DMA channel %d was expected to be assigned to our peripheral %d, but something else has assigned it to peripheral %d!", channelNumber, expectedPeripheralMap, peripheralMap);
  ...
  {
    DumpDMAPeripheralMap();
    LOG("DMA channel collision! Some other program has submitted a DMA task to our DMA channel %d! (DMA task at unknown control block address %p)", channelNumber, cbAddr);
    FATAL_ERROR("System is likely unstable now, rebooting is advised.");
  }
}
...
int InitDMA()
{
  ...
  LOG("Enabling DMA channels Tx:%d and Rx:%d", dmaTxChannel, dmaRxChannel);
...
void DumpDMAState()
{
  ...
  LOG("****** DMATX cbAddr: %p", dmaTx->cbAddr);
  ...
  LOG("****** DMARX cbAddr: %p", dmaRx->cbAddr);
}
...
static void memcpy_to_dma_and_prev_framebuffer_in_c(uint16_t *dstDma, uint16_t **dstPrevFramebuffer, uint16_t **srcFramebuffer, int numBytes, int *taskStartX, int width, int stride)
{
  ...
  {
    LOG("Performance warning: using slow memcpy_to_dma_and_prev_framebuffer_in_c() function. Check conditions in display.h that enable OFFLOAD_PIXEL_COPY_TO_DMA_CPP and configure to use that instead.");
